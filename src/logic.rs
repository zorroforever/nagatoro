...
use crate::{cmn, db, entity, kafka_listener};
...
pub async fn init() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Ok(_) = kafka_listener::init(&app_config, &pools).await {
        info!("success init kafka listener");
    };
    Ok(())
}
        ...
        match db::get_max_action_by_app_id_and_ext_id(db_pool, &app_id, ext_id).await {
            Ok((collection_action, cnt)) => {
                if cnt <= 0 {
                    ...
                    gaibu_show_data.action_promise_date = None;
                    ...
                    gaibu_show_data.action_promise_date = cmn::naive_date_time_to_naive_date(&(collection_action.promise_time));
